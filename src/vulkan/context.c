...
static char *sRequiredExtensions[2] = {0};
...
Purrr_Result create_logical_device(VkPhysicalDevice gpu, uint32_t graphicsQueueFamily, VkDevice *device) {
...
Purrr_Result _purrr_create_context_vulkan(Purrr_Context_Create_Info createInfo, _Purrr_Context_Vulkan **context) {
  ...
  _Purrr_Context_Vulkan *ctx = _purrr_malloc_item_vulkan(sizeof(*ctx));
  ...
  if (vkCreateInstance(&instanceCreateInfo, VK_NULL_HANDLE, &ctx->instance) != VK_SUCCESS) return PURRR_INTERNAL_ERROR;

  if ((result = choose_physical_device(ctx->instance, &ctx->gpu)) < PURRR_SUCCESS) return result;

  if ((result = query_queue_family(ctx->gpu, &ctx->graphicsQueueFamily)) < PURRR_SUCCESS) return result;

  if ((result = create_logical_device(ctx->gpu, ctx->graphicsQueueFamily, &ctx->device)) < PURRR_SUCCESS) return result;
